  ...
  template: `
    <div class="flex items-center justify-between">
      <div>
        ...
        <div class="text-sm text-slate-600">Talleres aplicados donde eres responsable.</div>
      </div>
      <button
...
export class TeacherWorkshopsPage {
  ...
  trackItem(_: number, item: TeacherWorkshopRow) {
    return item.applicationGroupId;
  }

  async openAttendance(item: TeacherWorkshopRow) {
    this.routeContext.setTeacherWorkshopAttendanceFocus({
      applicationId: item.applicationId,
      applicationGroupId: item.applicationGroupId,
    });
    await this.router.navigate(['/teacher/workshop-attendance']);
  }
  ...
  async load() {
    ...
    try {
      this.items = await firstValueFrom(
        this.http.get<TeacherWorkshopRow[]>('/api/teacher/workshops')
      );
    } catch (e: any) {
